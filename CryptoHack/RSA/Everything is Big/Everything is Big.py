from fractions import Fraction
import math
from Crypto.Util.number import long_to_bytes
from sympy import solve, Symbol

#Wiener's attack (since e is very large, there are chances that d might be really small)

def continued_fractions(frac):
    cf=[]
    while True:
        integer = math.floor(frac)
        cf.append(integer)
        frac = frac - integer
        if frac == 0:
            break
        frac = 1/frac
    return cf

def find_pqdtot(e,n):
    m = 12345 #Test plaintext
    c = pow(m,e,n)
    frac = Fraction(e,n)
    cf = continued_fractions(frac)
    for i in range(len(cf)):
        kd = cf[0]
        fr=Fraction(0,1)
        for j in reversed(range(1,i+1)):
            fr+=cf[j]
            fr=1/fr
        
        k=fr.numerator
        d=fr.denominator
        mm = pow(c,d,n)
        if not mm == m: #Checking if plaintext decrypted is equal to our test plaintext
            continue
        tot = (e*d-1)//k
        x=Symbol('x')
        p,q=solve(x**2 - (n-tot+1)*x + n, x)
        if math.floor(p) == p and math.floor(q) == q:
            return p,q,d,tot


N = 
e = 
c = 

p,q,d,tot = find_pqdtot(e,N)
pt = pow(c,d,N)
flag=long_to_bytes(pt).decode()
print(flag)